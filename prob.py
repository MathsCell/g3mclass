...
imposed=pa.DataFrame(np.array([nan, -1, nan], float).reshape((3, 1)), index=["a", "mean", "sd"]);
...
if False:
    ...
    plt.figure(200);
    plt.plot(xp, tls.fw(xp, par));
    plt.plot(xp, tls.fw_d1(xp, par));
    fw_d2=egrad(tls.fw_d1);
    plt.plot(xp, fw_d2(xp, par));
    plt.show(block=False);
if False:
...
for let in ("A", "B", "C", "D"):
    ref=data["Gene %s (ref)"%let];
    test=data["Gene %s (test)"%let];
